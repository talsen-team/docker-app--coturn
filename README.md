...
The server application coturn ready to run inside a docker container.

## how to use

To easily experiment with coturn, the following pre-requisites are preferred:
...
Any docker-compose--* tasks refer to the default [dockerfile](docker/server--coturn/default.docker) as well as to the [docker-compose](docker-compose/server--coturn/default.docker-compose) configuration if required for command execution.
...
- setup--*
  - [setup--generate-configuration](bash-commands--specific/setup--generate-configuration.sh)  
    Generates necessary coturn server configuration files.
